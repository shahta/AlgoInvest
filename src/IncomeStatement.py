...
class IncomeStatement:
    def __init__(self, ticker):
        ...
        self.__points = 0
        self.statements = self.ticker_eval.get_statement('income-statement')
        self.__net_income = [statement['netIncome'] for statement in self.statements]

    def evaluate_income_statement(self):
        print(colored(f'-------------------------------EVALUATING INCOME STATEMENTS FOR {self.ticker_name}-------------------------------------', 'green'))
        self.__eval_gross_profit_margin(self.statements)
        self.__eval_SGA_expenses(self.statements)
        self.__eval_RD_expenses(self.statements)
        self.__eval_depreciation_expenses(self.statements)
        self.__eval_interest_expense(self.statements)
        self.__eval_net_income(self.statements)
    ...
    def __eval_gross_profit_margin(self, statements:list):
        ...
        average_margin = self.ticker_eval.calculate_average(statements, 'grossProfit', 'revenue')
    ...
    def __eval_SGA_expenses(self, statements):
        ...
        average_SGA_expense = self.ticker_eval.calculate_average(statements, 'generalAndAdministrativeExpenses')
    ...
    def __eval_RD_expenses(self, statements):
        ...
        average_RD_expense = self.ticker_eval.calculate_average(statements, 'researchAndDevelopmentExpenses')
    ...
    def __eval_depreciation_expenses(self, statements):
        ...
        average_depreciation_expense = self.ticker_eval.calculate_average(statements, 'depreciationAndAmortization')
    ...
    def __eval_interest_expense(self, statements):
        ...
        average_interest_expense = self.ticker_eval.calculate_average(statements, 'interestExpense', 'operatingIncome')
    ...
    def __eval_net_income(self, statements):
        ...
        average_net_income = self.ticker_eval.calculate_average(statements, 'netIncome', 'revenue')
        ...
        trend = self.ticker_eval.determine_trend(statements, 'netIncome')
        if trend == 'increasing': 
            self.__points += 1
